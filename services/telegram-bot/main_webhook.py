...
def process_updates():
    ...
    while True:
        try:
            ...
            if update_data is None:  # Sentinel value to stop the thread
                break
            
            # Wait for bot to be initialized
            if bot_application is None:
                logger.warning("Bot not initialized yet, skipping update")
                continue
            ...
            # Queue is empty, this is normal - just continue waiting
            continue
            
        except Exception as e:
            # Only log if it's not a timeout from empty queue
    ...
    try:
        ...
        def set_webhook_async():
            try:
                loop = asyncio.new_event_loop()
                asyncio.set_event_loop(loop)
                result = loop.run_until_complete(bot_application.bot.set_webhook(webhook_url))
        ...
        thread = threading.Thread(target=set_webhook_async)
        thread.start()
        thread.join(timeout=10)  # 10 second timeout
        
        if result_container['error']:
            return {"error": result_container['error']}, 500
...
def initialize_bot():
    ...
    bot_application.add_handler(CommandHandler("price", bot_instance.price_command))
    bot_application.add_handler(CommandHandler("top10", bot_instance.top10_command))
    bot_application.add_handler(CommandHandler("analysis", bot_instance.analysis_command))
    bot_application.add_handler(CommandHandler("volume", bot_instance.volume_command))
    bot_application.add_handler(CommandHandler("cvd", bot_instance.cvd_command))
    bot_application.add_handler(CommandHandler("volscan", bot_instance.volscan_command))
    bot_application.add_handler(CommandHandler("oi", bot_instance.oi_command))
    bot_application.add_handler(CommandHandler("balance", bot_instance.balance_command))
    bot_application.add_handler(CommandHandler("positions", bot_instance.positions_command))
    bot_application.add_handler(CommandHandler("pnl", bot_instance.pnl_command))
    
    # Add error handler
    bot_application.add_error_handler(bot_instance.error_handler)
    ...
    try:
        ...
        # Setup bot commands
        loop.run_until_complete(setup_bot_commands(bot_application))
...
# Initialize bot application when module loads (for Gunicorn)
def startup():
    """Initialize bot and start update processor thread"""
    global update_processor_thread
    if initialize_bot():
        logger.info("Bot initialized successfully, starting update processor...")
        update_processor_thread = threading.Thread(target=process_updates, daemon=True)
        update_processor_thread.start()
